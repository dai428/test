"""基于NSGA-II的LCVR Stokes偏振计多目标优化论文 V5.0 — 核心期刊格式"""
...
def gen(doc):
    ...
    tbl(doc,
        ...
        [
            ['GA 6meas¹', '9.201', '10.646', '3.293', '0.04379', '0.04157'],
            ['2目标(无温漂)', '2.094', '0.349', '0.272', '0.01440', '0.01476'],
            ['3目标(有温漂)', '2.162', '0.471', '0.063', '0.01435', '0.01450'],
        ],
        cap='表3  Monte Carlo Stokes重建误差对比（500样本/温度点×36波长，基于非线性Haller模型评估）\n'
             '注：GA 6meas为单波长优化，全波段评估下CN较高',
    )
    body(doc,
        '表3和图5的结果表明：在36波长×11温度的全面评估中，三目标优化解在25°C时'
        'Total RMSE=0.01435，与2目标解（0.01440）基本持平；在全温域（−10~40°C）'
        '平均Total RMSE中，3目标解（0.01450）优于2目标解（0.01476），改善约1.8%。'
        '值得注意的是，上述Monte Carlo评估基于完整非线性Haller温漂模型（式(1)）而非'
        '优化阶段采用的线性近似，这验证了线性近似优化的结果在实际非线性温漂环境中仍然有效。'
        '为进一步评估统计显著性，在550nm单波长下以5000组独立Stokes样本进行了补充'
        '分析（采用双侧Welch t-test，不等方差t检验）。需注意，t-test仅覆盖550nm单'
        '波长点，而表3为36波长的全波段均值。结果表明：在−10°C极端温下，3目标解'
        'RMSE=0.01229，2目标解RMSE=0.01233，差异无统计学意义（p=0.66）；在25°C和'
        '40°C下，2目标解RMSE略优（p<0.001），差异幅度约3-4%。这说明在550nm单波长下'
        '3目标优化的代价是在室温下牺牲了少量精度以换取温度稳定性。全温域平均RMSE中'
        '3目标解优于2目标解（0.01450 vs 0.01476），这是因为全温域多波长均值受低温'
        '区域（−10~0°C）的CN恶化贡献更大——在低温区2目标解的平均CN从2.094上升至'
        '2.417（Δ=0.323），而3目标解仅上升至2.248（Δ=0.086），因此全波段均值反'
        '而改善。综合来看，非线性模型下ΔCN降幅达76.8%，而在相同噪声条件下'
        'RMSE改善幅度较小（1.8%），这是因为原系统（2目标解）在室温附近已接近最优工'
        '作区，测量误差主要受随机噪声主导。但ΔCN的改善对于保证仪器在宽温域内稳定运'
        '行具有重要的工程意义。')
...
def main():
    ...
    print(f'OK: {out}')
    import shutil
    for dst in ['/mnt/c/Users/45064/Desktop/LCVR_Paper_V5.0.docx',
                '/mnt/c/Users/45064/Desktop/论文/LCVR_Paper_V5.0.docx']:
        try:
