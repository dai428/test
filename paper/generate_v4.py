...
def gen(doc):
    ...
    abst(doc,
        ...
        'BCPN=0.471、极差ΔCN=0.089的综合性能。消融实验表明，加入温漂'
    ...
    body(doc,
        ...
        '（2）在6次测量配置下获得了综合性能均衡的膝点解；（3）通过消融实验和温度扫描'
        '分析，验证了多目标优化框架的有效性。')
    ...
    body(doc,
        '消融实验表明加入温漂目标后ΔCN降低75.4%，CN仅增3.2%。BCPN上升35%的物理本质'
    ...
    tbl(doc,
        ['方法', 'CN', 'BCPN', 'ΔCN', 'Total RMSE(25°C)', 'Total RMSE(全温域)'],
        [
            ['GA 6meas', '9.20', '10.65', '3.439', '0.04372', '0.04097'],
            ['2目标(无温漂)', '2.094', '0.349', '0.362', '0.01434', '0.01492'],
            ['3目标(有温漂)', '2.162', '0.471', '0.089', '0.01438', '0.01452'],
        ],
        cap='表3  Monte Carlo Stokes重建误差对比（500样本/温度点×36波长）'
    )
    body(doc,
        '表3和图5的结果表明：在36波长×11温度的全面评估中，三目标优化解在25°C时'
        'Total RMSE=0.01438，与2目标解（0.01434）基本持平；在全温域（−10~40°C）'
        '平均Total RMSE中，3目标解（0.01452）优于2目标解（0.01492），改善约2.7%。'
        '为进一步评估统计显著性，在550nm中心波长下以5000组独立Stokes样本进行了补充'
        '分析。采用双侧Welch t-test（不等方差t检验）检验2目标与3目标在各温度点的'
    ...
    body(doc,
        ...
        '宽温域内性能均衡的6次测量最优配置；（3）系统性的消融实验表明温漂鲁棒性与泊松'
        '噪声抑制之间存在明显权衡关系。')
    body(doc,
    ...
    body(doc,
        '（2）消融实验证明加入温漂目标后ΔCN降低75.4%（0.362→0.089），CN仅增3.2%。'
